const QUERIES = {
	get_non_friends = "select id from person where id not in \
					...
					and id!='$1' limit (select count(*)/2 as c from person where \
					id not in (select friend_id from friendship where person_id='$1') \
					and id not in (select person_id from friendship where friend_id='$1') \
					and id != '$1')",
